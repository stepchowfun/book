\documentclass[ebook,oneside]{memoir} % 6 by 9 inches

\usepackage[T1]{fontenc} % The font encoding
\usepackage[utf8]{inputenc} % The input encoding
\usepackage[USenglish]{babel} % Explicitly specify the hyphenation rules.
\usepackage[hidelinks,pdfa]{hyperref} % For clickable links
\usepackage{bookmark} % For `\bookmarksetup{startatroot}`
\usepackage[style=authoryear]{biblatex} % For citations and references
\usepackage{imakeidx} % For the index
\usepackage{microtype} % For micro-typography
\usepackage{pdfpages} % For the cover page
\usepackage{libertine} % The Libertine typeface
\usepackage[libertine]{newtxmath} % A math font based on Libertine
\usepackage{inconsolata} % The Inconsolata typeface

% Load the references.
\addbibresource{references.bib}

% Set up the index.
\makeindex[intoc]

% Define macros.
\input{macros}

\title{\booktitle}
\author{Stephan Boyer}
\date{}

\begin{document}
  \frontmatter

    % Book cover
    \includepdf{cover}

    % Copyright notice
    \pagebreak
    \hspace{0pt}
    \vfill
      \begin{center}
        Copyright \copyright{} \the\year{} Stephan Boyer. All rights reserved.
      \end{center}
    \vfill
    \hspace{0pt}
    \pagebreak

    % Preface
    \input{00_preface}

    % Table of contents
    \cleardoublepage
    \tableofcontents

  \mainmatter

    \part{Syntax}

      \chapter{Languages and grammars}

        A \emph{language}\index{language} is a set of strings.

      \chapter{Parsing}

        \section{Top-down parsing}

        \section{Bottom-up parsing}

      \chapter{Variable substitution}

    \part{Semantics}

      \chapter{Operational semantics}

        \section{Small-step operational semantics}

        \section{Big-step operational semantics}

      \chapter{Denotational semantics}

      \chapter{Axiomatic semantics}

      \chapter{Categorical semantics}

    \part{Types}

      \chapter{Simple types}

      \chapter{Sums and products}

      \chapter{Subtyping}

      \chapter{Parametric polymorphism}

      \chapter{Type operators}

      \chapter{Dependent types}

      \chapter{Recursive types}

      \chapter{Type inference}

    % End the last `\part{...}` so the references and index sections below appear at the top-level
    % of the PDF bookmarks.
    \bookmarksetup{startatroot}

  \backmatter

    % In the table of contents, add some vertical space between the last part above and the
    % references section below.
    \addtocontents{toc}{\vspace{\normalbaselineskip}}

    % References
    \printbibliography[heading=bibintoc,title=References]

    % Index
    \printindex
\end{document}
